  ...
  static BT::PortsList providedPorts()
  {
    return appendPorts(
      ...
      {BT::InputPort<std::string>("object_type"), BT::InputPort<std::string>("turning_direction"),
       BT::InputPort<double>("turning_angle")});
  }
...
protected:
  ...
  BT::NodeStatus publish_target_pose(Order order = Order::FIRST)
  {
    ...
    const auto interim_turning_angle = this->getInput<double>("turning_angle");
    ...
    if (!(order == Order::FIRST) && abs(total_turning_angle_deg) > abs(turning_angle) - inter_waypoint_angle_deg_) {
      waypoint_angle_deg = turning_angle - abs(total_turning_angle_deg);
    }
